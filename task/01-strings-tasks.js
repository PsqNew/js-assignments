...
function encodeToRot13(str) {
    let utf = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let rot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let findSymbol = x => utf.indexOf(x);
    let change = x => findSymbol(x) > -1 ? rot13[findSymbol(x)] : x;
    return str.split('').map(change).join('');
}
